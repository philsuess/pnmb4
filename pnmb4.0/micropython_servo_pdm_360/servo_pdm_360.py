...
class ServoPDM360:
    ...
    def __init__(
        self, pwm: PWM, min_us=500, max_us=3000, dead_zone_us=150, freq=50, invert=False
    ):
        self.pwm = pwm
        self.pwm.freq(freq)
    ...
    async def _turn_gen(
        ...
        time_ms: int = 0,
        force: int = None,
        start_smoothing: ServoSmoothBase = None,
        end_smoothing: ServoSmoothBase = None,
    ):
        ...
        if end_smoothing is not None:
            _de_func = (
                self.turn_ccv if self._curr_dir == self.DIRECTION_CCV else self.turn_cv
            )
